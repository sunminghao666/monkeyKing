...
public class TencentAISignSort {
    ...
    public static String getSignature(Map < String, String > params, String appKey) throws IOException {
        ...
        // 使用MD5对待签名串求签
        try {
            String sign = MD5Util.ENCRYPTION(baseString.toString());
            return sign;
        }
        catch (Exception ex) {
    ...
    public static String getyylockSignature(Map < String, String > params, String appKey) throws IOException {
        // 先将参数以其参数名的字典序升序进行排序
        Map < String, String > sortedParams = new TreeMap <>(params);
        Set < Map.Entry < String, String >> entrys = sortedParams.entrySet();
        // 遍历排序后的字典，将所有参数按"key=value"格式拼接在一起
        ...
        System.err.println("未拼接APPKEY的参数：" + baseString.toString());
        if (baseString.length() > 0) {
            baseString.deleteCharAt(baseString.length() - 1).append("&APPKEY=" + appKey);
        }
        System.err.println("拼接APPKEY后的参数：" + baseString.toString());
        // 使用MD5对待签名串求签
        try {
        ...
        for (Map.Entry < String, String > param : entrys) {
            ...
            if (param.getValue() != null && !"".equals(param.getKey().trim()) && !"sign".equals(param.getKey().trim())
                ...
                if (param.getKey().equals("text")) {
                    baseString.append(param.getKey().trim()).append("=")
                            .append(URLEncoder.encode(param.getValue().trim(), "GBK")).append("&");
                }
                else {
                    baseString.append(param.getKey().trim()).append("=")
                            .append(URLEncoder.encode(param.getValue().trim(), "UTF-8")).append("&");
