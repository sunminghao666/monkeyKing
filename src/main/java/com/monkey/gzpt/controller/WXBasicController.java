...
public class WXBasicController extends CommonController {
    ...
    public void getConfig(HttpServletRequest req, HttpServletResponse resp) {
        ...
        try {
            ...
            WxConfig config = WXapi.getJsapiTicket(basicDto.getBodyJSONObject().getString("url"));
            if (config == null) {
    ...
    public void authorize(HttpServletRequest req, HttpServletResponse resp) {
        Status status = new Status();
        try {
            BasicDto basicDto = new BasicDto();
            super.initParams(status, basicDto, req);
            TAFLog.info("静默授权code:" + basicDto.getBodyJSONObject().getString("code"));
            WXapi.wxAuthorize(basicDto.getBodyJSONObject().getString("code"), status);
    ...
    public void WXTemporaryMaterialDownload(HttpServletRequest req, HttpServletResponse resp) {
        Status status = new Status();
        try {
            BasicDto basicDto = new BasicDto();
            super.initParams(status, basicDto, req);
