 ...
 * imagePath,返回路径基本路径
 * 
 * callback全部上传完成后的回调 方法名不能是callback
 */
function initSwfupload(url, parm, sizeLimit, types, typsdesc, uploadLimit,
		imagePath, callback1) {
	...
	// 初始化 swfupload
	var settings = {
		flash_url : base.domain + "PC/component/upload/js/swfupload.swf",
		...
		// Button Settings
		button_image_url : base.domain
				+ "PC/component/upload/images/select-files-zh_CN.png",
...
function init(uploadLimit, sizeLimit) {
	...
	var uploadbody = '<div id="uploadBody" class="ke-dialog-upload">'
			...
			+ '		  <div class="ke-inline-block ke-swfupload-desc">允许用户同时上传&nbsp;<font color="red">'
			+ uploadLimit
			+ '</font>&nbsp;张图片，单张图片容量不超过<font color="red">'
			+ sizeLimit
			+ '</font></div>'
...
function insertAllevent() {
	...
	$("img[data-status='0']").each(function(i, obj) {
		...
		// KE.plugin['multiimage'].insert("ArticleBody",url,
...
function swfUploadLoaded() {
	...
	// clearTimeout(this.customSettings.loadingTimeout);
	addErrorMsg("", false);
	$("#" + this.customSettings.btnUpload_ID).click(function() {
		swfu.startUpload()
	});
}
...
function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
			message = "<font color='red'>数量超过"
					+ swfu.getSetting('file_upload_limit') + "张啦!</font>";
			break;
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			message = "<font color='red'>文件超过"
					+ swfu.getSetting('file_size_limit') + "啦!</font>";
			break;
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			message = "<font color='red'>文件不能为空哦!";
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			message = "<font color='red'>文件格式只能为"
					+ swfu.getSetting('fileTypes') + "!</font>";
			break;
		default:
...
function uploadSuccess(file, serverData) {
	try {
		...
		if (data.status == 0 || data.status == "0") {
			$("#" + file.id).find(".ke-img").attr("src",
					imageBasePath + "PC/images/2_04.png");
			progress.setStatus("上传成功");
			progress.toggleCancel();
			if (insertAllStatus) {
				if (swfu.getStats().successful_uploads > 0) {
					$("#" + swfu.customSettings.insertAll_Id).unbind("click")
							.bind("click", insertAllevent);
				}
...
function addReadyFileInfo(fileid, fileName, message) {
	var photoDiv = '<div class="ke-inline-block ke-item" id="'
			+ fileid
			+ '">'
			+ '<div class="ke-inline-block ke-photo">'
			+ '	<img src="'
			+ base.domain
			+ 'PC/component/upload/images/image.png" class="ke-img" data-status="-1" width="80" height="80" />'
			...
			+ '        		 </div>' + '				<div class="ke-message">' + message
			+ '</div>' + '   	 	</div>' + '    	</div>'
			+ '  <div class="ke-name" alt="' + fileName + '">' + fileName
			+ '</div>' + '</div>';

	$("#" + swfu.customSettings.photoContainer_Id).append(photoDiv);
	$("#" + fileid).find(".ke-photo").bind("mouseover", photoMouseOver).bind(
			"mouseout", photoMouseOut);
	$("#" + fileid).find(".ke-delete").bind("click", function() {
		deleteFile(fileid)
	});
}
...
FileProgress.prototype.setProgress = function(percentage) {
	$(this.fileProgressWrapper).find(".ke-progressbar-bar-inner").css("width",
			percentage + "%");
	$(this.fileProgressWrapper).find(".ke-progressbar-percent").text(
			percentage + "%");
};
...
FileProgress.prototype.toggleCancel = function(show) {
	if (typeof show == 'undefined') {
		$(this.fileProgressWrapper).find(".ke-progressbar").css("display",
				"none");
		$(this.fileProgressWrapper).find(".ke-message").css("display", "none");
	} else {
		$(this.fileProgressWrapper).find(".ke-progressbar").css("display",
				show ? "block" : "none");
		$(this.fileProgressWrapper).find(".ke-message").css("display",
				show ? "none" : "block");
	}
};
